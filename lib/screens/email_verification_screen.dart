...
class _EmailVerificationScreenState extends State<EmailVerificationScreen> {
  ...
  Future<void> _sendVerificationEmail({bool silent = false}) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _status = 'Verification email sent to ${user.email}. Check Spam/Junk/Promotions too.');
  ...
  String _authMessage(FirebaseAuthException e) {
    switch (e.code) {
      case 'too-many-requests': return 'Too many requests. Thodi der baad Resend try karo.';
      case 'network-request-failed': return 'Internet connection check karo.';
      case 'user-disabled': return 'This account is disabled.';
      case 'verification-email-timeout': return 'Firebase response timeout. Internet check karke Resend try karo.';
      default: return 'Firebase Error: ${e.code}\n${e.message ?? 'Unknown error'}';
    }
  }
  ...
  Future<void> _checkVerification() async {
    ...
    try {
      ...
      debugPrint('VERIFICATION CHECK: emailVerified=${refreshed.emailVerified}');

      if (!refreshed.emailVerified) {
        if (mounted) {
          setState(() => _status = 'Email abhi verify nahi hua. Pehle verification link open karo.');
        }
        return;
      }
      ...
      if (mounted) setState(() => _status = 'Firebase Error: ${e.code}\n${e.message ?? ''}');
    } catch (e) {
  ...
  Future<void> _createProfileAfterVerification(User user) async {
    ...
    final name = prefs.getString('pending_signup_name') ?? user.displayName ?? '';
    ...
    if (!existing.exists) {
      ...
      if (usernameQuery.docs.any((doc) => doc.id != user.uid)) {
        if (mounted) setState(() => _status = 'Username @$username is already taken. Logout karke another username se account create karo.');
        return;
      }
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  ...
                  const Card(child: Padding(padding: EdgeInsets.all(16), child: Text('Inbox ke saath Spam, Junk aur Promotions folder bhi check karo. Link open karne ke baad yahan “I Verified My Email” press karo.'))),
                  if (_status != null) ...[
                    const SizedBox(height: 16),
                    Text(_status!, textAlign: TextAlign.center),
                  ],
                  const SizedBox(height: 24),
                  SizedBox(width: double.infinity, height: 50, child: FilledButton.icon(onPressed: _checking ? null : _checkVerification,
                    icon: _checking ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.verified_outlined),
                    label: Text(_checking ? 'Checking...' : 'I Verified My Email'))),
                  const SizedBox(height: 12),
                  SizedBox(width: double.infinity, height: 50, child: OutlinedButton.icon(onPressed: _sending ? null : () => _sendVerificationEmail(),
                    icon: _sending ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.refresh),
                    label: Text(_sending ? 'Sending...' : 'Resend Verification Email'))),
                  const SizedBox(height: 12),
                  TextButton.icon(onPressed: _logout, icon: const Icon(Icons.logout), label: const Text('Use another account')),
                ],
              ),
            ),
          ),
