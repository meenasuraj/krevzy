...
class _CustomizationScreenState extends State<CustomizationScreen> {
  ...
  Future<void> _pickApp(ImageSource source) async {
    ...
    try {
      final url = await KrevzyBackgroundService.pickAndUploadAppBackground(
        source: source,
      );
      if (!mounted) return;
      if (url != null) {
  ...
  Future<void> _pickKeyboard() async {
    ...
    try {
      final url =
          await KrevzyBackgroundService.pickAndUploadKeyboardBackground();
      if (!mounted) return;
      if (url != null) {
        setState(() => _keyboardPhoto = url);
      }
    } catch (e) {
      _error(e);
    } finally {
  ...
  void _error(Object e) {
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text('Background upload failed: $e')));
  }
      ...
      body: Stack(
        children: [
          ...
          ListView(
            ...
            children: [
              const Text(
                'Make KREVZY feel like yours',
                style: TextStyle(fontSize: 25, fontWeight: FontWeight.w800),
              ),
              const SizedBox(height: 6),
              Text(
                'Use the KREVZY pastel identity or your own photo.',
                style: TextStyle(
              ...
              _tile(
                Icons.auto_awesome,
                'KREVZY default',
                'Pink • lavender • sky gradient',
                _appType == 'krevzy',
                () async {
                  await KrevzyBackgroundService.setAppPreset('krevzy');
                  setState(() {
                    _appType = 'krevzy';
                    _appPhoto = null;
                  });
              ...
              _tile(
                ...
                _appPhoto == null
                    ...
                    : 'Your custom photo is active',
                _appType == 'photo',
                () => _pickApp(ImageSource.gallery),
              ),
              _tile(
                Icons.camera_alt_outlined,
                'Take a photo',
                'Use your camera as the app background',
                false,
                () => _pickApp(ImageSource.camera),
              ),
              ...
              _tile(
                ...
                'Per-chat background',
                'Each conversation can have its own background. Open a chat and choose Chat Theme.',
                false,
                null,
              ),
              const SizedBox(height: 24),
              ...
              _tile(
                ...
                _keyboardPhoto == null
                    ? 'Customizes the KREVZY message composer area'
                    : 'Your custom composer background is active',
                _keyboardPhoto != null,
                _pickKeyboard,
              ),
              ...
              const Text(
                'Note: KREVZY can customize its own composer UI; it cannot change the system Gboard keyboard.',
  ...
  Widget _preview(String url) => Padding(
