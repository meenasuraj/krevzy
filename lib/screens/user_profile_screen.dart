...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  Future<void> _toggleFollow(bool currentlyFollowing) async {
    ...
    try {
      if (currentlyFollowing) {
        await FollowService.unfollow(widget.userId);
      } else {
  ...
  Widget _buildFollowButton() {
    ...
    return StreamBuilder<bool>(
      stream: FollowService.followingStream(widget.userId),
      builder: (context, snapshot) {
        ...
        return SizedBox(
          ...
          child: FilledButton.icon(
            onPressed: isWaiting || _isFollowLoading
                ? null
                : () {
  ...
  Widget _buildPostsSection() {
    return Container(
      ...
      child: const Column(
        children: [
          Icon(Icons.grid_on_outlined, size: 48),
          SizedBox(height: 12),
          Text('No posts to show yet.', textAlign: TextAlign.center),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    final displayName = widget.name.isNotEmpty ? widget.name : 'KREVZY User';

    final displayUsername = widget.username.isNotEmpty
        ? '@${widget.username}'
        : '@user';
        ...
        child: Column(
          children: [
            ...
            Row(
              ...
              children: [
                _buildStat(value: widget.postsCount, label: 'Posts'),
                ...
                StreamBuilder<int>(
                  ...
                  builder: (context, snapshot) {
                    return _buildStat(
                      value: snapshot.data ?? widget.followersCount,
                      label: 'Followers',
                ...
                StreamBuilder<int>(
                  ...
                  builder: (context, snapshot) {
                    return _buildStat(
                      value: snapshot.data ?? widget.followingCount,
                      label: 'Following',
