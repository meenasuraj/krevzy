...
class YourActivityScreen extends StatelessWidget {
  ...
  Future<void> _clear(BuildContext context) async { try{await KrevzySocialSettingsService.clearActivity();if(context.mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Activity cleared.')));}catch(e){if(context.mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Unable to clear activity: $e')));}}
  @override
