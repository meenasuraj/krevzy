...
class _TagsMentionsScreenState extends State<TagsMentionsScreen> {
  ...
  bool approval = false, loading = true;
  final choices = ['Everyone', 'People you follow', 'No one'];
  @override
  void initState() {
    super.initState();
    _load();
  ...
  Future<void> _load() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        tags = d['tags']?.toString() ?? 'Everyone';
        mentions = d['mentions']?.toString() ?? 'Everyone';
        approval = d['manual_tag_approval'] as bool? ?? false;
        loading = false;
      });
    } catch (_) {
  ...
  Widget row(String t, String k, String v) {
    return ListTile(
      title: Text(t),
      subtitle: Text(v),
      trailing: DropdownButton<String>(
        value: choices.contains(v) ? v : choices.first,
        items: choices
            .map((e) => DropdownMenuItem(value: e, child: Text(e)))
            .toList(),
        onChanged: (x) {
          if (x == null) return;
          setState(() => k == 'tags' ? tags = x : mentions = x);
          _save(k, x);
        },
      ),
    );
  ...
  Widget build(BuildContext c) => Scaffold(
    appBar: AppBar(title: const Text('Tags and mentions')),
    body: loading
        ? const Center(child: CircularProgressIndicator())
        : ListView(
            children: [
              row('Who can tag you', 'tags', tags),
              row('Who can mention you', 'mentions', mentions),
              SwitchListTile(
                title: const Text('Manual approval'),
                subtitle: const Text(
                  'Approve tagged content before it appears on your profile.',
                ),
                value: approval,
                onChanged: (v) {
                  setState(() => approval = v);
                  _save('manual_tag_approval', v);
                },
              ),
            ],
          ),
