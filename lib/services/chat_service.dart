...
class ChatService {
  ...
  // ---------------------------------------------------------------------------
  // KREVZY PAY (SANDBOX / TEST ONLY)
  // ---------------------------------------------------------------------------

  /// Stores a sandbox payment message in the same chat message collection.
  /// No real-money transaction is performed by this method.
  static Future<void> sendPaymentMessage({
    ...
    if (chatData == null) {
      throw Exception('Chat data is unavailable.');
    }
    ...
    if (!participants.contains(receiverId)) {
      throw Exception('Payment receiver is not a participant in this chat.');
    }

    final unreadCounts =
        Map<String, dynamic>.from(chatData['unreadCounts'] ?? <String, dynamic>{});

    final currentOtherUnread = unreadCounts[receiverId] is num
        ? (unreadCounts[receiverId] as num).toInt()
        : 0;

    unreadCounts[receiverId] = currentOtherUnread + 1;
    unreadCounts[uid] = unreadCounts[uid] is num
    ...
    batch.update(chatRef, {
      'lastMessage': 'Payment • $currency ${amount.toStringAsFixed(2)} • $statusText',
      'lastMessageTime': FieldValue.serverTimestamp(),
      'lastMessageSenderId': uid,
      'unreadCounts': unreadCounts,
      'typingUsers.$uid': 'none',
    });

    await batch.commit();

    // Notification failure must never undo a successfully stored payment message.
    try {
      await NotificationService.createMessageNotification(
        receiverUserId: receiverId,
        chatId: chatId,
        message: 'Payment • $currency ${amount.toStringAsFixed(2)}',
      );
    } catch (_) {}
  }
