...
class NotificationService {
  ...
  getLikeActivities() {
    ...
    return _notifications(user.uid)
        .where('type', whereIn: ['like', 'unlike'])
        .orderBy('createdAt', descending: true)
        .limit(50)
        .snapshots()
  ...
  static Future<void> createNotification({
    ...
    if (receiverId.isEmpty) {
      throw Exception('Notification receiver UID is empty.');
    }
    ...
    final notificationRef = _notifications(receiverId).doc();
  ...
  static Future<void> markLikeActivitiesAsRead() async {
    final user = _auth.currentUser;
  ...
  static Future<void> createMessageNotification({
    ...
    final username = senderData['username']?.toString().trim() ?? '';

    final name = senderData['name']?.toString().trim() ?? '';

    final photoUrl = senderData['photoUrl']?.toString().trim() ?? '';
  ...
  static Future<void> markChatNotificationsAsRead(String chatId) async {
    final user = _auth.currentUser;

    if (user == null || chatId.trim().isEmpty) {
      return;
    }
    ...
    final unreadDocs = snapshot.docs.where(
  ...
  static Future<void> createTestNotification() async {
    ...
    final notificationRef = _notifications(user.uid).doc();
    ...
    await notificationRef.set({
      ...
      'postId': '',
      'chatId': '',
      'message': 'krevzy notification test successful',
      'isRead': false,
      'createdAt': FieldValue.serverTimestamp(),
    });
  }
  ...
  static Future<void> markAsRead(String notificationId) async {
    ...
    await _notifications(user.uid).doc(notificationId).update({'isRead': true});
  }
  ...
  static Future<void> deleteNotification(String notificationId) async {
    ...
    await _notifications(user.uid).doc(notificationId).delete();
  }
}
