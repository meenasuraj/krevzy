...
class _PostCardState extends State<PostCard>
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              saved ? 'Post saved.' : 'Post removed from saved posts.',
            ),
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Unable to save post: $e')));
      }
    } finally {
  ...
  Future<void> _loadLikeStatus() async {
    try {
      final liked = await PostService.hasLiked(_post.id);
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      ...
      if (_isLiked) {
        await _likeAnimationController.forward(from: 0);
      }
    } catch (e) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Like update nahi ho saka: $e'),
  ...
  void _openComments() {
    Navigator.of(
      context,
    ).push(MaterialPageRoute(builder: (_) => CommentsScreen(postId: _post.id)));
  }
  ...
  void _openProfile() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => PublicProfileScreen(userId: _post.userId),
      ),
    );
  }
  ...
  Future<void> _deletePost() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Post delete nahi ho saka: $e'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    } finally {
  ...
  Widget _buildAvatar() {
    final photo = _post.userPhotoUrl.trim();

    if (photo.isNotEmpty) {
      return CircleAvatar(radius: 20, backgroundImage: NetworkImage(photo));
    }

    final name = _post.userName.trim().isNotEmpty
        ? _post.userName.trim()
        : _post.username.trim();
  ...
  Widget build(BuildContext context) {
    final displayName = _post.userName.trim().isNotEmpty
        ? _post.userName.trim()
        : _post.username.trim();

    final username = _post.username.trim();
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
                ...
                Expanded(
                  child: InkWell(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          displayName.isNotEmpty ? displayName : 'KREVZY User',
          ...
          // MEDIA
          // ------------------------------------------------------
          if (_post.mediaUrl.trim().isNotEmpty)
            AspectRatio(
              aspectRatio: 1,
              child: Image.network(
                _post.mediaUrl,
          ...
          Padding(
            ...
            child: StreamBuilder<int>(
              stream: PostService.getLikeCount(_post.id),
              builder: (context, likeSnapshot) {
                final likesCount = likeSnapshot.data ?? 0;
